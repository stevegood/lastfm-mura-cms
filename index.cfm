...
<cfsilent>
	...
	<cfset qResult.location = "" /> 
	<cfset qResult.distance = "" />
	<cfset qResult.tagList = "" />
	
	<cfquery name="qGetLastFM" datasource="#application.configbean.getDatasource()#">
		SELECT username,song_count,location,distance,tagList
		FROM lastfm_config
	...
	<cfif qGetLastFM.recordCount>
		...
		<cfset qResult.location = qGetLastFM.location /> 
		<cfset qResult.distance = qGetLastFM.distance />
		<cfset qResult.tagList = qGetLastFM.tagList />
	</cfif>
		...
		<cfif qGetLastFM.recordCount>
			<cfquery name="qUpdate" datasource="#application.configbean.getDatasource()#">
				...
				SET username = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#form.username#" />,
					...
					distance = <cfqueryparam cfsqltype="CF_SQL_INTEGER" value="#form.distance#" />,
					tagList = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#form.tagList#" />
				WHERE siteId = <cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#session.siteId#" />
			</cfquery>
		<cfelse>
			<cfquery name="qUpdate" datasource="#application.configbean.getDatasource()#">
				INSERT INTO lastfm_config(username,song_count,siteid,location,distance,tagList)
				VALUES(	<cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#form.username#" />,
						<cfqueryparam cfsqltype="CF_SQL_INTEGER" value="#form.song_count#" />,
						<cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#session.siteId#" />,
						<cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#form.location#" />,
						<cfqueryparam cfsqltype="CF_SQL_INTEGER" value="#form.distance#" />,
						<cfqueryparam cfsqltype="CF_SQL_VARCHAR" value="#form.tagList#" />)
			</cfquery>	
		</cfif>
		<cfset qResult.username = form.username />
		<cfset qResult.song_count = form.song_count />
		<cfset qResult.location = form.location /> 
		<cfset qResult.distance = form.distance />
		<cfset qResult.tagList = form.tagList />
	</cfif>
</cfsilent> 
	...
	<cfoutput>
		...
		<div>
			<form name"lastfm" action="index.cfm" method="post" onsubmit="return validate(this);">
				...
				<ul>
					<li>
						<label for="location">Event City</label><input type="text" id="location" name="location" value="#qResult.location#" required="true" message="Please enter your location." />
					</li>
					<li>
						<label for="distance">Search Radius (km)</label><input type="text" id="distance" name="distance" value="#qResult.distance#" validate="numeric" required="false" />
					</li>
					<li>
						<label for="tagLst">Event Tag List</label><input type="text" id="tagLst" name="tagList" value="#qResult.tagList#" required="false" size="60" maxlength="255" />
					</li>
				</ul>
				<div class="buttons">
